...
def getQbound(nSim):
    if nSim == 1:
        ...
        print(
            f"Warning: Neutrino luminosity for simulation {nSim} not hard-coded. Defaulting to the luminosity for simulation 1."
        )
        Qbound = Qbound1
...
def getTrappingLumiOne(xChi, R, T):
    ...
    if xChi > 1e-2:
        fac, _ = itg.quad(
            lambda x: x**2 * (x**2 - xChi**2) ** 0.5 / (np.exp(x) + 1),
            xChi,
            np.inf,
        )
...
def getRadiusSphere(mChi, R, T, nSim=1, out=False):
    ...
    if iCrit != 0:
        if out:
            print(
                "Properties of blackbody radiation (from i={0}): r = {1:.2f} km, T = {2:.2f} MeV".format(
                    iCrit, R[iCrit] / km2invMeV, T[iCrit]
                )
            )
        return iCrit
    else:
