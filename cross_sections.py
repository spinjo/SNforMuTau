...
def sigma0DM_VV(epsL, epsDM, y1):
    return sigma0DM_V(epsL, epsDM, y1, 1, 0, 1, 0)
def sigma0DM_LV(epsL, epsDM, y1):
    return sigma0DM_V(epsL, epsDM, y1, 1/2, -1/2, 1, 0)
...
def sigmaPropagator_0DM(mL, mChi, T, y1, iSigma, mZp=1, gL=1, gChi=1, **kwargs):
    ...
    sigmaVal = sigmaArr0DM[iSigma]( (mL/T)**2, (mChi/T)**2, y1) *T**2 #factor T^2 to get correct dimensions
...
def sigma1DM_VV(epsL, epsDM, epsZ, y1):
    return sigma1DM_V(epsL, epsDM, epsZ, y1, 1, 0, 1, 0)
def sigma1DM_LV(epsL, epsDM, epsZ, y1):
    return sigma1DM_V(epsL, epsDM, epsZ, y1, 1/2, -1/2, 1, 0)
sigmaArr1DM=np.array([sigma1DM_VV, sigma1DM_LV])

def sigmaPropagator_1DM(mL, mChi, T, y1, iSigma, mZp=1, gL=1, gChi=1): 
    if(y1*T**2 < (mL + mChi)**2):
        return 0.
    sigmaVal=sigmaArr1DM[iSigma]( (mL/T)**2, (mChi/T)**2, (mZp/T)**2, y1) *gL**2*gChi**2 /T**2
    return sigmaVal
...
def sigmaPropagator_DMself_t(mChi, T, y1, iSigma, gChi=1, gL=1, mZp=1, **kwargs):
    if(y1*T**2 < (2*mChi)**2):
        return 0.
    sigmaVal=sigmaArr1DM[iSigma]( (mChi/T)**2, (mChi/T)**2, (mZp/T)**2, y1) *gChi**4 /T**2
    return sigmaVal
